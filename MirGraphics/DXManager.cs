...
namespace Client.MirGraphics
{
    ...
    /// <summary>
    /// PixelShader（着色器，用来定义画图所用的像素?）
    /// Texture(文理)它和Material(材质)绑在一起,表示是物体表面的
    /// Surface表面-->缓冲区(物理)   Texture材质-->逻辑上图案(逻辑上)
    /// 常常采用Surface加载位图作为Texture.而Surface(表面),多和缓冲区相联系.你可以创建后台,表面在上画图,提交到主表面(屏幕).
    /// Surface这个可以理解成缓冲区，缓冲，一般定义2个缓冲，相互交换用于显示即可。
    /// Texture这个是实际的图案等信息，需要放入到缓冲区进行显示的
    /// 大概关系入下Device->2个Surface交换显示->多个Texture推送到缓冲区
    /// </summary>
    class DXManager
    {
